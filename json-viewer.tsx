      ...
      if (!item.processed) {
        ...
        if (Array.isArray(item.value)) {
          ...
          for (let i = item.value.length - 1; i >= 0; i--) {
            ...
            stack.push({ value: item.value[i], context: "array", depth: (item.depth || 0) + 1 });
            if (useFormatting) {
              stack.push({ value: indentStr.repeat((item.depth || 0) + 1), context: "indent" });
            }
          }
        } else {
          // Add opening brace
          ...
          for (let i = filteredEntries.length - 1; i >= 0; i--) {
            ...
            // Push value
            stack.push({ value: val, context: "object", depth: (item.depth || 0) + 1 });
            // Push key with colon
            if (useFormatting) {
              stack.push({ value: ": ", context: "separator" });
            } else {
        ...
        if (Array.isArray(item.value)) {
          ...
          // Add closing brace
          const entries = Object.entries(item.value);
          const filteredEntries = entries.filter(([key]) => key !== "__visited__");
...
function safeStringify(data, indent) {
  try {
    ...
    return iterativeStringify(data, indent);
  }
}
