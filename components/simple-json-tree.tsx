...
export function SimpleJsonTree({
  ...
  const [initialExpandedPaths, setInitialExpandedPaths] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    ...
    const collapsedLevelsChanged =
      ...
      Array.from(collapsedLevels).some(
        (level) => !lastCollapsedLevelsRef.current.has(level)
      );
      ...
      if (match) {
        ...
        setTimeout(() => {
          ...
          for (let i = 0; i < parts.length - 1; i++) {
            ...
            console.log(
              `📊 PARENT DATA AT "${parentPath}":`,
              parentData ? Object.keys(parentData).slice(0, 5) : "null"
            );
            ...
            if (parentData && typeof parentData === "object") {
              if (Array.isArray(parentData)) {
                ...
                console.log(
                  `📋 ARRAY: hiding siblings of index ${matchIndex} out of ${parentData.length} items`
                );
                ...
                console.log(
                  `🗝️ OBJECT: hiding siblings of key "${part}" out of keys:`,
                  keys.slice(0, 10)
                );
                keys.forEach((key) => {
                  if (key !== part) {
          ...
          console.log(
            "❌ SIBLINGS TO HIDE:",
            Array.from(siblingsToHide).slice(0, 10)
          );
          ...
          setTimeout(() => {
            ...
            if (element) {
              ...
              console.log(
                ...
                Array.from(document.querySelectorAll('[id^="json-node-"]')).map(
                  (el) => el.id
                )
              );
            }
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (currentData && typeof currentData === "object") {
        if (Array.isArray(currentData)) {
          ...
          console.log(
            `📋 ARRAY ACCESS: index ${index} of ${currentData.length}`
          );
          currentData = currentData[index];
        } else {
          console.log(
            `🗝️ OBJECT ACCESS: key "${part}" in keys:`,
            Object.keys(currentData).slice(0, 10)
          );
        ...
        console.log(
          ...
          currentData
            ? typeof currentData === "object"
              ? `${Array.isArray(currentData) ? "array" : "object"} with ${
    ...
    (parentPath: string, type: "above" | "below", matchPath: string) => {
      ...
      if (Array.isArray(parentData)) {
        ...
        parentData.forEach((_, index) => {
          if (
            (type === "above" && index < matchIndex) ||
            (type === "below" && index > matchIndex)
          ) {
            const siblingPath =
        ...
        keys.forEach((key, index) => {
          if (
            (type === "above" && index < matchKeyIndex) ||
            (type === "below" && index > matchKeyIndex)
          ) {
            const siblingPath =
              parentPath === "root" ? key : `${parentPath}.${key}`;
            pathsToToggle.add(siblingPath);
      ...
      setHiddenSiblings((prev) => {
        const newSet = new Set(prev);
        const isCurrentlyHidden = Array.from(pathsToToggle).some((path) =>
          newSet.has(path)
        );
  ...
  const getPreviewText = useCallback((obj: any, maxItems = 5) => {
    if (Array.isArray(obj)) {
      return `[0..${Math.min(maxItems - 1, obj.length - 1)}]${
        obj.length > maxItems ? "..." : ""
      }`;
  ...
  const handleStringClick = useCallback((value: string, path: string) => {
    console.log(`🔗 STRING CLICKED: "${value}" at path "${path}"`);
    ...
    (
      ...
      const isHidden = (() => {
        if (isArray) {
          ...
          const testPath =
            parentPath === "root" ? testKey : `${parentPath}.${testKey}`;
          return hiddenSiblings.has(testPath);
        }
      })();
        ...
        >
          <div style={{ paddingLeft: `${indent}px` }} className="flex-1">
            <Button
              ...
              onClick={() => {
                console.log(
                  `🔄 TOGGLING SIBLINGS: ${type} for path "${parentPath}" with match "${matchPath}"`
                );
                toggleSiblings(parentPath, type, matchPath);
              }}
  ...
  const renderJsonValue = useCallback(
    (
      ...
      const shouldRenderToggles =
        currentMatchPath && path === currentMatchPath && parentPath;
      ...
      if (value === null) {
        ...
        return (
          <div key={elementKey}>
            {shouldRenderToggles &&
              renderSiblingToggle(parentPath, "above", path, level)}
            <div
              className={`flex items-start py-1 px-2 rounded transition-colors ${
                ...
                isCurrentMatch && !isKeyMatch
                  ? "bg-yellow-200 dark:bg-yellow-800/50 ring-2 ring-yellow-400"
                  : ""
              }`}
              id={`json-node-${safeId}`}
            ...
            {shouldRenderToggles &&
              renderSiblingToggle(parentPath, "below", path, level)}
          </div>
        );
      }
      ...
      if (Array.isArray(value)) {
        ...
        if (isExpanded && value.length > 0) {
          value.forEach((item, index) => {
            const itemPath = `${path}[${index}]`;
            const renderedItem = renderJsonValue(
              item,
              index,
              itemPath,
              level + 1,
      ...
      if (typeof value === "object" && value !== null) {
        ...
        if (isExpanded && keys.length > 0) {
          keys.forEach((objKey) => {
            ...
            const renderedProp = renderJsonValue(
              value[objKey],
              objKey,
              propPath,
              level + 1,
  ...
  return (
    <div className="relative h-full">
      ...
      <div className="font-mono text-sm p-4 h-full overflow-auto">
        {renderJsonValue(data, null, "root", 0)}
      </div>
      <StringModal
